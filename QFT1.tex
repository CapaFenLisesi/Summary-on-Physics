...
A similar behavior is possible in an asymptotically free theory. If the $\beta$ function behaves in the vicinity of the fixed point as 
\[\beta \approx B(\lambda - \lambda_*)\] 
...
The Callan–Symanzik equation is most interesting in the massless limit, and for a theory with a zero of the beta function at a nonzero value of $\lambda$. So, let us suppose that $\beta(\lambda_*)$ for some $\lambda_* \neq 0$. Then, for $\lambda = \lambda_*$ and $m=0$, the Callan–Symanzik equation for two point Green function in momentum space becomes
\[\left( \frac{\partial}{\partial \ln \mu} + 2 \gamma_{\phi}(\lambda_*)\right)\mathcal{D}(k^2) = 0\]
The solution is
\[\mathcal{D} = \frac{C(\lambda_*)}{k^2}\left( \frac{\mu^2}{k^2} \right )^{-\gamma_{\phi}(\lambda_*)}\]
where $C(\lambda_*)$ is an integration constant. (We used the fact that $\mathcal{D}(k^2)$ has mass dimension $-2$ to get the $k^2$ dependence in addition to the $\mu$ dependence.) Thus the naive scaling law $\mathcal{D}(k^2) \sim k^{-2}$ is changed to $\mathcal{D}(k^2) \sim k^{-2[1-\gamma_{\phi}(\lambda_*)]}$. This has applications in the theory of critical phenomena.
