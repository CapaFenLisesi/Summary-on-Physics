...
the field equation can then be written as so-called Maxwell equations:
\begin{eqnarray}
	...
	\bm{\nabla} \cdot \bm{B} &=& 0 \nonumber
\end{eqnarray}
The equation of motion of the charged particle can be written as so-called Lorentz force equations:
...
here,
\[f_n = \frac{1}{T} \int_{0}^{T} f(t)e^{in\omega_0} dt\]
...
In wave zone, we have
\[\bm{B}_{\omega} = i\bm{k} \times \bm{A}_{\omega} \quad \bm{E} = \frac{1}{\omega} \left( \bm{k} \times \bm{A}_{\omega}\right) \times \bm{k}\]
...
t-\bm{n}\cdot\bm{v}_0]} \bm{v}_0 dt\]
\[\bm{B}_{\omega}(\bm{R}_0) = \frac{e\omega e^{i\omega
R_0}}{4\pi R_0}  \int_{-\infty}^{\infty} e^{i\omega[
t-\bm{n}\cdot\bm{v}_0]}  \bm{n} \times d\bm{r}_0\]
...
\[d\mathcal{E}_{\omega} = \frac{|(\ddot{\bm{d}})_{\omega}|^2}{3\pi} = \frac{\omega^4}{3\pi}|\bm{d}_{\omega}|^2 \frac{d\omega}{2\pi}\]
...
Consider the particle in the system of reference in which the particle is at rest at a given moment; in this system of reference we can apply low velocity approximation. Here, we have
...
So,we have
\[\bm{F}_{rad} = \frac{e^2}{6\pi}\cdot{\bm{a}}\]
...
Here, $\cos\Theta = \cos\phi\sin\theta$, $\Theta$ is the angle between the direction of incident light and scatted light.

\subsubsection{Scattering by bound charges}
The dynamic equation of the bound charges are
