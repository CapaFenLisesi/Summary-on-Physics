...
\[\mathcal{L}_{kin} = il^{\dagger i} \overline{\sigma}^{\mu}(D_{\mu}l)_i + i\bar{e}^{\dagger}\overline{\sigma}^{\mu}D_{\mu}\bar{e}\]
...
\[\mathcal{L}_{Yuk} = -y\epsilon^{ij}\phi_i l_j \bar{e} + \mathrm{h.c.}\]
...
\[\mathcal{L}_{Yuk} = -\frac{1}{\sqrt{2}}y(v+H)(l_2\bar{e} + \mathrm{h.c.})\]
It is now convenient to assign new names to the $SU(2)$ components of $l$,
\[l = \begin{pmatrix}
\nu \\ e
\end{pmatrix} \]
So, we have
\[\mathcal{L}_{Yuk} = -\frac{1}{\sqrt{2}}y(v+H)\overline{\mathcal{E}}\mathcal{E}\]
where we have defined a Dirac field for the electron
\[\mathcal{E} \equiv \begin{pmatrix}
e \\ \bar{e}^{\dagger}
\end{pmatrix} \]
We see that the electron has acquired a mass $m_e = \frac{yv}{\sqrt{2}}$, while neutrino has remained massless.
...
\[\mathcal{N}_L \equiv P_L \mathcal{N} = \begin{pmatrix}
\nu \\ 0
\end{pmatrix}\]
We can think of $\mathcal{N}_L$ as a Dirac field; for example, the neutrino kinetic term can be written as $i\overline{\mathcal{N}_L} \slashed{\partial} \mathcal{N}_L$.
\\ \\
...
\end{pmatrix} \]
and
\[g_2A^3_{\mu}T^3 + g_1 B_{\mu} Y = e(T^3+Y)A_{\mu} + e(\cot\theta_W T^3 - \tan\theta_W Y)Z_{\mu}\]
...
\[g_2A^3_{\mu}T^3 + g_1 B_{\mu} Y = eQA_{\mu} + \frac{e}{s_W c_W}( T^3 -  s_W^2Q)Z_{\mu}\]
In terms of the four-component fields, we have
\[(g_2A^3_{\mu}T^3 + g_1 B_{\mu} Y) \mathcal{E} = \left[-eA_{\mu} + \frac{e}{s_W c_W}( -\frac{1}{2}P_L +  s_W^2)Z_{\mu} \right] \mathcal{E}\]
\[(g_2A^3_{\mu}T^3 + g_1 B_{\mu} Y) \mathcal{N}_L = \frac{e}{s_W c_W} (+\frac{1}{2}) Z_{\mu}\mathcal{N}_L \]
The couplings of the gauge fields to the leptons can be written as
\[\mathcal{L}_{int} = \frac{1}{\sqrt{2}}g_2W_{\mu}^{+} J^{-\mu} + \frac{1}{\sqrt{2}}g_2W_{\mu}^{-} J^{+\mu} +  \frac{e}{s_W c_W} Z_{\mu}J_Z^{\mu} + eA_{\mu}J^{\mu}_{EM}\] 
where
\begin{eqnarray}
J^{+\mu} &\equiv & \overline{\mathcal{E}}_L \gamma^{\mu} \mathcal{N}_L \nonumber \\
J^{-\mu} &\equiv & \overline{\mathcal{N}}_L \gamma^{\mu} \mathcal{E}_L \nonumber \\
J_Z^{\mu} &\equiv & J_3^{\mu} - s_W^2 J^{\mu}_{EM} \nonumber \\
J_3^{\mu} &\equiv & \frac{1}{2}\overline{\mathcal{N}}_L \gamma^{\mu} \mathcal{N}_L - \frac{1}{2}\overline{\mathcal{E}}_L \gamma^{\mu} \mathcal{E}_L \nonumber \\
J^{\mu}_{EM} &\equiv &  -\overline{\mathcal{E}} \gamma^{\mu} \mathcal{E}_L \nonumber
\end{eqnarray}
\\ \\
...
\[\mathcal{L}_{kin} = il^{\dagger i}_I \overline{\sigma}^{\mu}(D_{\mu})_{i}^{\phantom{i}j} l_{jI} + i\bar{e}_I^{\dagger}\overline{\sigma}^{\mu}D_{\mu}\bar{e}_I\]
where the repeated generation index is summed. The most general Yukawa term we can write down now reads
\[\mathcal{L}_{Yuk} = -\epsilon^{ij}\phi_i l_{jI}y_{IJ} \bar{e}_{J} + \mathrm{h.c.}\]
...
\[\mathcal{L}_{kin} = iq^{\dagger \alpha i} \overline{\sigma}^{\mu} (D_{\mu}q)_{\alpha i} + i\bar{u}^{\dagger}_{\alpha}\overline{\sigma}^{\mu}(D_{\mu}\bar{u})^{\alpha} + i\bar{d}^{\dagger}_{\alpha}\overline{\sigma}^{\mu}(D_{\mu}\bar{d})^{\alpha}\]
...
\[\mathcal{L}_{Yuk} = -y'\epsilon^{ij}\phi_i q_{\alpha j} \bar{d}^{\alpha} - y" \phi^{\dagger i}q_{\alpha i} \bar{u}^{\alpha} + \mathrm{h.c.}\]
...
In unitary gauge, we replace $\phi_1$ with $\frac{1}{\sqrt{2}}(v+H)$, where $H$ is the real scalar field representing the physical Higgs boson, and $\phi_2$ with zero. The Yukawa term becomes
\[\mathcal{L}_{Yuk} = -\frac{1}{\sqrt{2}}y'(v+H)q_{\alpha 2}\bar{d}^{\alpha} - \frac{1}{\sqrt{2}}y''(v+H)q_{\alpha 1}\bar{u}^{\alpha} + \mathrm{h.c.}\]
It is now convenient to assign new names to the $SU(2)$ components of $q$,
\[q = \begin{pmatrix}
u \\ q
\end{pmatrix} \]
Then we have
\[\mathcal{L}_{Yuk} = -\frac{1}{\sqrt{2}}y'(v+H) \overline{\mathcal{D}}_{\alpha}\mathcal{D}_{\alpha} -\frac{1}{\sqrt{2}}y''(v+H) \overline{\mathcal{U}}_{\alpha}\mathcal{U}_{\alpha}\]
...
\[m_d = \frac{y'v}{\sqrt{2}} \quad m_u = \frac{y''v}{\sqrt{2}}\]
Now we express the covariant derivatives in terms of the $W_{\mu}^{\pm}$ , $Z_{\mu}$ , and $A_{\mu}$ fields. We have
\[g_2 A^1_{\mu}T^1 + g_2 A^2_{\mu}T^2 = \frac{g_2}{\sqrt{2}} \begin{pmatrix}
0 & W_{\mu}^+ \\ W^{-}_{\mu} & 0
\end{pmatrix} \]
...
\[(g_2A^3_{\mu}T^3 + g_1 B_{\mu} Y) \mathcal{U} = \left[ + \frac{2}{3}eA_{\mu} + \frac{e}{s_W c_W}( \frac{1}{2}P_L -\frac{2}{3}  s_W^2)Z_{\mu} \right] \mathcal{U}\]
\[(g_2A^3_{\mu}T^3 + g_1 B_{\mu} Y) \mathcal{U} = \left[ - \frac{1}{3}eA_{\mu} + \frac{e}{s_W c_W}( -\frac{1}{2}P_L +\frac{1}{3}  s_W^2)Z_{\mu} \right] \mathcal{U}\]
The coupings of the electroweak gauge fields to the quarks is
\[\mathcal{L}_{int} = \frac{1}{\sqrt{2}}g_2W_{\mu}^{+} J^{-\mu} + \frac{1}{\sqrt{2}}g_2W_{\mu}^{-} J^{+\mu} +  \frac{e}{s_W c_W} Z_{\mu}J_Z^{\mu} + eA_{\mu}J^{\mu}_{EM}\] 
where we have defined the currents
\begin{eqnarray}
J^{+\mu} &\equiv & \overline{\mathcal{D}}_L \gamma^{\mu} \mathcal{U}_L \nonumber \\
J^{-\mu} &\equiv & \overline{\mathcal{U}}_L \gamma^{\mu} \mathcal{U}_L \nonumber \\
J_Z^{\mu} &\equiv & J_3^{\mu} - s_W^2 J^{\mu}_{EM} \nonumber \\
J_3^{\mu} &\equiv & \frac{1}{2}\overline{\mathcal{U}}_L \gamma^{\mu} \mathcal{U}_L - \frac{1}{2}\overline{\mathcal{D}}_L \gamma^{\mu} \mathcal{D}_L \nonumber \\
J^{\mu}_{EM} &\equiv &  +\frac{2}{3}\overline{\mathcal{U}} \gamma^{\mu} \mathcal{U}_L - \frac{1}{3}\overline{\mathcal{D}} \gamma^{\mu} \mathcal{D}_L \nonumber
\end{eqnarray}
...
\[\mathcal{L}_{kin} = iq^{\dagger\alpha i I} \overline{\sigma}^{\mu}(D_{\mu})_{\alpha i}^{\phantom{\alpha i}\beta j} q_{\beta jI} + i\bar{u}_{\alpha I}^{\dagger}\overline{\sigma}^{\mu} (D_{\mu})^{\alpha}_{\phantom{\alpha}\beta}\bar{u}_I^{\beta} + i\bar{d}_{\alpha I}^{\dagger}\overline{\sigma}^{\mu} (D_{\mu})^{\alpha}_{\phantom{\alpha}\beta}\bar{d}_I^{\beta}\]
where the repeated generation index is summed. The most general Yukawa term we can write down now reads
\[\mathcal{L}_{Yuk} = -\epsilon^{ij}\phi_i q_{\alpha jI}y'_{IJ} \bar{d}^{\alpha}_{J} -\phi^{\dagger i} q_{\alpha jI}y''_{IJ} \bar{u}^{\alpha}_{J} + \mathrm{h.c.}\]
$y'$ and $y''$ are complex $3 \times 3$ matrices. In unitary gauge, this becomes
\[\mathcal{L}_{Yuk} = -\frac{1}{\sqrt{2}}(v+H) d_{\alpha I}y'_{IJ} \bar{d}^{\alpha}_{J} -\frac{1}{\sqrt{2}}(v+H) u_{\alpha I}y''_{IJ} \bar{u}^{\alpha}_{J} + \mathrm{h.c.}\]
...
\[J^{+\mu} = \overline{\mathcal{D}}_{LI} (V^{\dagger})_{IJ} \gamma^{\mu} \mathcal{U}_{LJ}\]
\[J^{-\mu} = \overline{\mathcal{U}}_{LI} V_{IJ} \gamma^{\mu} \mathcal{D}_{LJ}\]
...
Since the time-reversal operator T is antiunitary, the charged currents do not transform in a simple way under time reversal. This implies that the charged current terms in $\mathcal{L}_{int}$ are not time-reversal invariant; hence the electroweak interactions violate time-reversal symmetry.
